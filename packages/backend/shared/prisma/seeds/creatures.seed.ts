...
export async function SeedCreatures() {
    ...
    if(creatures.length < 100){
        for (let creature_data of creatures_data) {
            let creature_actions = []

            console.log(creature_data)

            if(creature_data.action){
                for(let action of creature_data.action){
                    let new_action : {name: string, text: string, attack: string} = {name: '', text: '', attack: ''}
            ...
            if(creature_data.trait){
                ...
                for(let trait of creature_data.trait){
                    let new_trait : {name: string, text: string, attack: string} = {name: '', text: '', attack: ''}
            ...
            if(creature_data.immune){
                let parts = creature_data.immune.split(';')
            ...
            if(creature_data.vulnerable){
                let parts = creature_data.vulnerable.split(';')
            ...
            if(creature_data.resist){
                let parts = creature_data.resist.split(';')
            ...
            let speeds_strings = creature_data.speed.split(',')
            let creature_speeds : {walk?: number, fly?: number, swim?: number, climb?: number, burrow?: number} = {}
            for(const speed_string of speeds_strings){
                let split_string = speed_string.split(' ')
            ...
            if(creature_data.skill){
                let creature_skills_list = creature_data.skill.split(', ')
            ...
            await prisma.creature.create({
                data: {
                    ...
                    actions: creature_actions ? {
                        connectOrCreate: creature_actions.map((action) => ({where: {description: action.text}, create: {description: action.text, name: action.name, is_template: false}}))
                    } : {},  
                    traits: creature_traits ? {
                        connectOrCreate: creature_traits.map((trait) => ({where: {description: trait.text}, create: {description: trait.text, name: trait.name, is_template: false}}))
                    } : {},
                    immunities: creature_immunities ? {connect: creature_immunities.map((immunity) => ({name: immunity.trim().toLowerCase()}))} : {},
                    resistances: creature_resistances ? {connect: creature_resistances.map((resistance) => ({name: resistance.trim().toLowerCase()}))} : {},
                    vulnerabilities: creature_vunlerabilities ? {connect: creature_vunlerabilities.map((vunlarability) => ({name: vunlarability.trim().toLowerCase()}))} : {},
                    biome_relation: {
